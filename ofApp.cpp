	...
	for (float x = -200; x <= 200; x += span) {
		...
		for (float y = -200; y <= 200; y += span) {
			...
			for (float z = -200; z <= 0; z += span) {
				...
				auto noise_value = ofNoise(x * noise_param, y * noise_param, z * noise_param + ofGetFrameNum() * 0.05);

				if (noise_value < threshold) { continue; }

				auto noise_value_1 = ofNoise((x - span) * noise_param, y * noise_param, z * noise_param + ofGetFrameNum() * 0.05);
				auto noise_value_2 = ofNoise(x * noise_param, (y + span) * noise_param, z * noise_param + ofGetFrameNum() * 0.05);
				auto noise_value_3 = ofNoise((x + span) * noise_param, y * noise_param, z * noise_param + ofGetFrameNum() * 0.05);
				auto noise_value_4 = ofNoise(x * noise_param, (y - span) * noise_param, z * noise_param + ofGetFrameNum() * 0.05);
